...
export default function SongsPage() {
  ...
  function Map(props) {
    ...
    function initMap() {
      ...
      var counter = 0;
      if (data.length > 0) {
        data.forEach(housingMark => {
          if (housingMark.Latitude && housingMark.Longitude && counter < 10) {
            const marker = new window.google.maps.Marker({
              position: { lat: housingMark.Latitude, lng: housingMark.Longitude },
              map: map,
              title: housingMark["Address"],
            });

            marker.addListener('click', () => {
              const infowindow = new window.google.maps.InfoWindow({
                content: housingMark["Address"],
              });
  ...
  const search = () => {
    ...
    fetch(`http://${process.env.REACT_APP_SERVER_HOST}:${process.env.REACT_APP_SERVER_PORT}/getrankhousing?Neighborhood=${neighborhood}` +
  ...
  return (
    ...
    <Container>
      <h2>Search Neighborhood for Housing</h2>
